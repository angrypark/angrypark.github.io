...
## Weak Annotator
이 문제를 해결하기 위해서 이 논문에서는 기존에 사람의 pair를 1로 학습하는 건 같지만 나머지를 0으로 학습하지 않고 weak annotator 모델로 예측한 예측값을 사용합니다. 기본적인 framework는 annotator에 어떤 모델을 사용해도 되지만 여기서는 대용량의 사람 사이의 대화로 pre-trained된 Seq2Seq을 사용하였습니다.
...
위의 학습방법은 positive pair의 matching score와 negative에 대한 annotator의 점수의 차이를 최대한 크게 벌리는 데에 그 목적이 있습니다. 즉, 기존 방법들과 negative와 positive의 score 간의 차이를 키우는 데에는 같지만, weak annotation을 통해 negative간의 차이를 준다는 장점이 있다고 생각합니다. 대표적으로 semantic하게는 유사하지만 negative인 것들을 예로 들 수 있겠죠.
