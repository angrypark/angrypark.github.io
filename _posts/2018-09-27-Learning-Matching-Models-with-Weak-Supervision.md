...
위의 학습방법은 positive pair의 matching score와 negative에 대한 annotator의 점수의 차이를 최대한 크게 벌리는 데에 그 목적이 있습니다. 즉, 기존 방법들과 negative와 positive의 score 간의 차이를 키우는 데에는 같지만, weak annotation을 통해 negative간의 차이를 준다는 장점이 있다고 생각합니다. 한 예로 semantic하게 유사하지도 않은 negative에게는 더 낮은 점수를 주고, semantic하게는 유사할 경우에는 어느 정도의 점수를 주고 학습을 시키게 됩니다. 논문에서는 이를 
